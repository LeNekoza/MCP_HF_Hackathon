...
def load_database_config() -> Dict[str, Any]:
    ...
    if all(os.getenv(key) for key in ['NEON_HOST', 'NEON_DATABASE', 'NEON_USER', 'NEON_PASSWORD']):
        return {
            "database": {
    ...
    try:
        with open('neon_config.json', 'r') as f:
            config = json.load(f)
            print("WARNING: Using neon_config.json file. Consider using environment variables for production.")
            return config
